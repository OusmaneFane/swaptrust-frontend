...
export function PhoneInputSection({ register, errors, watch }: PhoneInputProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-input border border-emerald-500/20 bg-emerald-500/[0.06] p-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs text-ink-secondary">
            Vous serez notifié à chaque étape de vos échanges directement sur WhatsApp.
            Aucune application supplémentaire nécessaire.
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        {errors.phoneMali ? (
          <p className="mt-1 text-xs text-danger">{errors.phoneMali.message}</p>
        ) : null}
      </div>
      ...
      <div>
        ...
        {errors.phoneRussia ? (
          <p className="mt-1 text-xs text-danger">{errors.phoneRussia.message}</p>
        ) : null}
      </div>

      <p className="text-xs text-ink-faint">
        Au moins un numéro est requis. Les deux sont recommandés si vous êtes entre les
        deux pays.
      </p>
    </div>
  );
